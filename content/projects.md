...
[Spectral Gravity Wave Drag (SGWD) Module – GitHub Merge (gw_ussp_mod.F90)](https://github.com/MetOffice/lfric_apps/commit/db82bec425dc45f757e5a575340225d7741e0d6a)
...
A major part of this work has been developing reproducible workflows for building, running, profiling, and validating GPU-enabled OpenFOAM cases across different compilers, dependency stacks, memory modes, and hardware layouts. This includes comparing standard CPU builds, stdpar GPU builds, and stdpar multicore builds, while capturing enough provenance to trace results back to the executable, environment, solver configuration, runtime settings, job layout, logs, validation outputs, and profiling artefacts used.

I have also worked on the practical performance engineering needed to make the GPU path run effectively on real multi-GPU systems. This includes experimenting with MPI rank placement, GPU binding, CPU and thread affinity, rank-per-GPU layouts, OpenMP settings, and decomposition strategies to avoid oversubscription, reduce unnecessary synchronisation, and improve use of the available CPU-GPU topology. On unified architectures such as GH200 and MI300A-class systems, this also means paying close attention to how CPU placement, memory behaviour, and accelerator affinity interact with the stdpar execution model.

Memory management and solver configuration have been central to the work. I investigated UMPIRE-backed memory management, managed-memory settings, and alternative GPU memory-mode configurations. These experiments helped separate genuine GPU-port performance behaviour from configuration artefacts, first-step startup costs, memory-footprint issues, and CPU-side solver bottlenecks.

Profiling and validation are used throughout to guide the optimisation work. I use Nsight Systems, OpenFOAM logs, full-domain field comparisons, repeated timing runs, and generated performance summaries to assess where time is spent across CUDA kernel execution, CUDA API overheads, CPU orchestration, MPI synchronisation, decomposition overhead, memory allocation, and solver phases that do not yet benefit strongly from stdpar execution. This helps identify whether performance limits are coming from kernel granularity, affinity and binding choices, memory behaviour, MPI scaling, solver setup, or platform-specific runtime effects.

This activity has also resulted in contributions back to platform, vendor, and project teams when issues were traced to compiler, toolchain, or runtime behaviour rather than project code. One example is an issue observed with NVHPC 24.11 on Grace/AArch64, where the device-link helper (`pgacclnk`) compiles its internal link stub with small PIC (`-fpic`) during device link. On AArch64 this can lead to a small GOT and, for large shared libraries such as `libOpenFOAM.so`, trigger a GOT overflow and final link failure. I documented the behaviour, captured supporting trace evidence showing the injected `-fpic` flag, and provided a minimal reproducer to assist investigation.
